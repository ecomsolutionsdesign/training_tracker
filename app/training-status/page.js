...
export default function TrainingStatusPage() {
    ...
    const [selectedPosition, setSelectedPosition] = useState('All');   // NEW
    ...
    const fetchPositions = async () => {    // NEW
        try {
        ...
        employees.forEach(employee => {
            ...
            if (recentAttendance) {
                ...
                attended.push({
                    ...
                    trainerName: schedule?.trainer?.name || 'N/A'   // updated from trainerName
                });
            } else {
    ...
    const filterEmployeesBySearch = (employeeList) => {
        ...
        if (selectedPosition !== 'All') {
            filtered = filtered.filter(emp =>
                (emp.position?._id?.toString() || emp.position?.toString()) === selectedPosition
            );
        }
        ...
        <Layout>
            <div className="max-w-7xl mx-auto p-6">
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                    {/* Row 1: Topic picker + Department + Position */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div className="relative">
                            ...
                            {isTopicOpen && (
                                <div className="fixed inset-0 z-40" onClick={() => setIsTopicOpen(false)} />
                            )}
                        </div>
                    ...
                    {selectedTopic && (
                        <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
                            ...
                            <p className="text-sm text-blue-800 mt-1">
                                Showing employees who attended this topic in the last 3 months (since {new Date(new Date().setMonth(new Date().getMonth() - 3)).toLocaleDateString('en-GB', {
                                    day: '2-digit', month: '2-digit', year: 'numeric'
                                })})
                            </p>
                            <p className="text-xs text-blue-700 mt-1">
                                💡 Use department, position filter and search to narrow down results
                            </p>
                        </div>
                    )}
                </div>
                ...
                {selectedTopic ? (
                                        ...
                                        <div key={emp._id} className="bg-green-50 border border-green-200 rounded-lg p-4">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-green-700">{emp.department} · {emp.position?.name || emp.role}</p>
                                                </div>
                                        ...
                                        <div key={emp._id} className="bg-red-50 border border-red-200 rounded-lg p-4">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-red-700">{emp.department} · {emp.position?.name || emp.role}</p>
                                                </div>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-12 text-center">
                        ...
                        <h3 className="text-xl font-semibold text-gray-700 mb-2">Select a Training Topic to Begin</h3>
                        <p className="text-gray-500">
                            Choose a training topic from the dropdown above to see attended and pending employees.
                        </p>
                    </div>
                )}
                        ...
                        {(selectedDepartment !== 'All' || selectedPosition !== 'All' || searchTerm) && (
                            <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                                <p className="text-sm text-yellow-800">
                                    📊 <strong>Filtered View:</strong> Showing results for
                                    {selectedDepartment !== 'All' ? ` ${selectedDepartment} dept` : ' all depts'}
                                    {selectedPosition !== 'All' ? ` · ${positions.find(p => p._id === selectedPosition)?.name || ''} position` : ''}
